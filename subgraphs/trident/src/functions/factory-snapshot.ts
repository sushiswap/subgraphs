...
import { Factory, FactoryDaySnapshot, FactoryHourSnapshot } from '../../generated/schema'
import { BIG_DECIMAL_ZERO, BIG_INT_ONE, BIG_INT_ZERO, DAY_IN_SECONDS, HOUR_IN_SECONDS, PairType } from '../constants'
...
export function updateFactorySnapshots(
  ...
  updateFactoryHourSnapshot(event, globalFactory, volume)
  updateFactoryDaySnapshot(event, globalFactory, volume)
...
function updateFactoryHourSnapshot(
  ...
  let id = generateFactoryHourSnapshotId(factory.id, event.block.timestamp)
