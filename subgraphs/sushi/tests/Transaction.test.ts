...
test('Transfer', () => {
  ...
  const transactionId = transferEvent.transaction.hash.toHex() + '-' + transferEvent.logIndex.toHex()
  ...
  assert.fieldEquals('Transaction', transactionId, 'to', receiver.toHex())
...
test('Zero address is sender sets transaction type to MINT', () => {
  const receiver = Address.fromString('0x0000000000000000000000000000000000000b0b')
  const amount = BigInt.fromString('1337')
  let transferEvent = createTransferEvent(ADDRESS_ZERO, receiver, amount)
...
test('Zero address is receiver sets transaction type to BURN', () => {
