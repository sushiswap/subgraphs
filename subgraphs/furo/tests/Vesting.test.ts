...
test('Created vesting contains expected fields', () => {
  ...
  assert.fieldEquals('Vesting', id, 'cliffShares', CLIFF_AMOUNT.toString())
  assert.fieldEquals('Vesting', id, 'stepShares', STEPS_AMOUNT.toString())
  assert.fieldEquals('Vesting', id, 'remainingShares', TOTAL_AMOUNT.toString())
...
test('Mint transaction does NOT update the vesting recipient', () => {
  ...
  onTransferVesting(transactionEvent)
