...
import { CONSTANT_PRODUCT_POOL_FACTORY_ADDRESS, STABLE_POOL_FACTORY_ADDRESS, PairType } from '../constants'
...
export function onDeployPool(event: DeployPool): void {
  ...
  if (event.params.factory == CONSTANT_PRODUCT_POOL_FACTORY_ADDRESS) {
    // FIXME: enable these later, just disabled for tesing stable pools atm
