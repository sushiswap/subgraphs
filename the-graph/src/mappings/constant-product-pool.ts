...
export function onMint(event: MintEvent): void {
  ...
  const mint = new Mint(
    "constant-product:" +
      ...
      ":" +
      pool.transactionCount.toString()
  );
...
export function onSync(event: Sync): void {
  log.debug(
    "[ConstantProduct] onSync...... event.reserve0: {} event.reserve1: {}",
    [event.params.reserve0.toString(), event.params.reserve1.toString()]
  );
  ...
  log.debug(
    "[ConstantProduct] onSync [BEFORE] pool.reserve0: {} pool.reserve1: {}",
  ...
  log.debug(
    "[ConstantProduct] onSync [AFTER] pool.reserve0: {} pool.reserve1: {}",
    [pool.reserve0.toString(), pool.reserve1.toString()]
  );
